...
template<class Ele, class Keytype> class list {
 ...
 public:
  ...
  void combine(list<Ele, Keytype> *target_list);
  void push(Ele *e);
  Ele *pop();
...
template<class Ele, class Keytype> 
void
list<Ele,Keytype>::combine(list<Ele,Keytype> *target_list){
  Ele *e_tmp = my_head;
  Ele *target_element;

  while (e_tmp) {
    target_element = target_list->lookup(e_tmp->key());
    if (target_element) {
      target_element->count += e_tmp->count;
      e_tmp = e_tmp->next;
    }
    else {
      Ele *e_next;
      e_next = e_tmp->next;
      target_list->push(e_tmp);
      e_tmp = e_next;
    }
  }
}
