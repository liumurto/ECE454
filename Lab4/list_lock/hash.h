...
#define TOTAL_NUMBER_OF_LISTS 1 << 14
#define HASH_INDEX(_addr,_size_mask) (((_addr) >> 2) & (_size_mask))

pthread_mutex_t list_locks[TOTAL_NUMBER_OF_LISTS];
...
hash<Ele,Keytype>::lock_list(Keytype the_key){
  int hash_index = HASH_INDEX(the_key,my_size_mask);
  pthread_mutex_lock(&list_locks[hash_index]);
  return;
} 
...
hash<Ele,Keytype>::unlock_list(Keytype the_key){
  int hash_index = HASH_INDEX(the_key,my_size_mask);
  pthread_mutex_unlock(&list_locks[hash_index]);
  return;
} 
